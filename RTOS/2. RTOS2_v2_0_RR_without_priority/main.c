/*----------------------------------------------------------------------------
 * Name:      main.c
 * Purpose: 	CMSIS-RTOS test program v2.0 - Round Robin with no priority
 * Designer: 	Aleksandar
 *----------------------------------------------------------------------------*/
...
void delay(int count){
  ...
  for(i=0;i<count;i++)
  {
    for(j=0;j<20200;j++); // ~ 1 milisecond delay
  }	
}
void delay2(int count){
  ...
  for(i=0;i<count;i++)
  {
    for(j=20200;j>0;j--); // ~ 1 milisecond delay
  }	
}
void delay3(int count){
	unsigned int i,j;
  for(i=0;i<count;i++)
  {
    for(j=0;j<40400;j++)
		j++; // ~ 1 milisecond delay
  }	
}
...
int main (void) {
  ...
  LPC_GPIO1->FIODIRH = 0x0034;     // Configure the Pins on GPIO1: 18,20,21 as OUTPUT;
